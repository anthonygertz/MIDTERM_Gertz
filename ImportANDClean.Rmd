...
noaa_select <- noaa_raw %>%
  ...
  rename(event_id = EVENT_ID, state = STATE, year = YEAR, month = MONTH_NAME, 
         ...
         event = EVENT_TYPE, state_fips = STATE_FIPS, county_fips = CZ_FIPS)
...
noaa_select <- noaa_select %>%
  ...
  mutate(damage_cpi = if_else(damage_mod == 'M',
                             1000000 * damage_num,
                             1000 * damage_num))
```
...
#Creates a new data frame that merges the CPI data by year and creates a new column that shows damage values by 2020 CPI dollars

floods <- merge(noaa_select, cpi, 
                by.x = 'year', by.y = 'Year', 
                all.x = TRUE, all.y = FALSE) %>%
          mutate(damage_cpi = damage_cpi * cpi_adj)
```
...
data <- dir_create('data')

#Writes both necessary csv files to the working directory
write.csv(census_full, 'data/census_full.csv', row.names = FALSE)
write.csv(floods, 'data/floods.csv', row.names = FALSE)
```
